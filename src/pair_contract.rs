pub use crate::constants::{
    ETH_MAINNET_HTTP, GAS_PRICE_GWEI, MAX_ITERATIONS, MINIMUM_PROFIT_MARGIN,
    SUSHISWAP_GAS_ESTIMATE, SUSHISWAP_ROUTER, TOKEN_SHIBA_INU_ADDRESS, TOKEN_WETH_ADDRESS,
    UNISWAP_GAS_ESTIMATE, UNISWAP_ROUTER,
};
...
pub async fn listen_swap_events(
    ...
    // Create instances of the Uniswap and Sushiswap routers
    let uniswap_router = UniswapV2Router02::new(
        UNISWAP_ROUTER.parse::<H160>().unwrap(),
        uniswap_pair_contract.client(),
    );
    let sushiswap_router = SushiswapRouter02::new(
        SUSHISWAP_ROUTER.parse::<H160>().unwrap(),
        sushiswap_pair_contract.client(),
    );
    let weth = TOKEN_WETH_ADDRESS.parse::<U256>().unwrap();
    let shib = TOKEN_SHIBA_INU_ADDRESS.parse::<U256>().unwrap();

    let event_signature = "Swap(address,uint256,uint256,uint256,uint256,address)";
    let _event_topic = H256::from_slice(keccak256(event_signature).as_ref());
    ...
    while let Some(log) = stream.next().await {
        ...
        // let data: [u8; 32 * 5] = log.data.0.try_into()?;
        // let amount0_in = U256::from(data[0..32].try_into()?);
        ...
        println!(
            "UNISWAP reserves after swap: \n reserve0: {} \n reserve1: {}",
            uniswap_reserve0, uniswap_reserve1
        );
        println!(
            "SUSHISWAP reserves after swap: \n reserve0: {} \n reserve1: {}",
        ...
        let gas_cost =
            calculate_gas_cost(UNISWAP_GAS_ESTIMATE, SUSHISWAP_GAS_ESTIMATE, GAS_PRICE_GWEI);

        match direction {
            ArbitrageDirection::BuyUniswapSellSushiswap => {
                println!("Buy on Uniswap, sell on Sushiswap.");
                let optimal_trade_amount = calculate_optimal_trade_amount(
                    uniswap_reserve0,
                    sushiswap_reserve0,
                    gas_cost.to_string().parse::<f64>().unwrap(),
                    MINIMUM_PROFIT_MARGIN,
                    MAX_ITERATIONS,
                );
                println!("Optimal trade amount: {:?}", optimal_trade_amount);
                println!(
                    "gas cost: {} and gas cost in eth: {}",
                    gas_cost,
                    wei_to_eth(gas_cost)
                );
                if let Some(amount) = optimal_trade_amount {
                    // Calculate the expected output amounts for both platforms
                    let uniswap_output: U256 = uniswap_router
                        .get_amount_out(amount, weth, shib)
                        .call()
                        .await?;
                    let sushiswap_output: U256 = sushiswap_router
                        .get_amount_out(uniswap_output, shib, weth)
                        .call()
                        .await?;

                    println!("Uniswap input: {} eth", wei_to_eth(amount));
                    println!("Sushiswap output: {}", wei_to_eth(sushiswap_output));
                    println!("gas cost: {}", wei_to_eth(gas_cost));
                    // Calculate the would-be profit
                    let would_be_profit =
                        sushiswap_output - uniswap_output - (gas_cost * U256::from(2));

                    println!(
                            "Optimal trade amount: {}, with an estimated profit of {:.4}% with the amount of {}",
                            amount,
                            (would_be_profit.to_string().parse::<f64>().unwrap() / amount.to_string().parse::<f64>().unwrap()) * 100.0,
                            wei_to_eth(would_be_profit)
                        );

                    println!(
                        "buy from uniswap for {} and sell on sushiswap for {}",
            ...
            ArbitrageDirection::BuySushiswapSellUniswap => {
                ...
                if let Some(amount) = optimal_trade_amount {
                    ...
                    let sushiswap_output: U256 = sushiswap_router
                        ...
                        .await?;
                    let uniswap_output: U256 = uniswap_router
                        .get_amount_out(sushiswap_output, shib, weth)
                        .call()
                        .await?;
                    println!("Sushiswap input: {} eth", wei_to_eth(amount));
                    println!("Uniswap output: {} eth", wei_to_eth(amount));
                    println!("gas fees in eth: {}", wei_to_eth(gas_cost));
                    // Calculate the would-be profit
                    let would_be_profit =
                        uniswap_output - sushiswap_output - (gas_cost * U256::from(2));
                    ...
                    println!(
                        "buy from sushiswap for {} and sell on uniswap for {}",
                        wei_to_eth(sushiswap_output),
                        wei_to_eth(uniswap_output)
                    );
                } else {
...
pub async fn get_sushiswap_pair_address<M: Middleware>(
    factory_contract: &SushiswapV2Factory<Provider<Ws>>,
    token_a: &str,
    token_b: &str,
...
pub async fn get_uniswap_pair_address<M: Middleware>(
    factory_contract: &UniswapV2Factory<Provider<Ws>>,
    token_a: &str,
    token_b: &str,
) -> Result<Address, Box<dyn Error + Send + Sync>> {
    let pair_address: Address = factory_contract
...
pub async fn get_uniswap_pair_contract<M: Middleware>(
    provider: Arc<M>,
...
pub async fn get_sushiswap_pair_contract<M: Middleware>(
    provider: Arc<M>,
    pair_address: Address,
) -> SushiswapV2Pair<M> {
    SushiswapV2Pair::new(pair_address, provider)
}

pub enum Pair<M: Middleware> {
    UniswapV2Pair(UniswapV2Pair<M>),
    SushiswapV2Pair(SushiswapV2Pair<M>),
}

pub async fn get_reserves<M: Middleware + 'static>(
    pair_contract: &Pair<M>,
) -> Result<(U256, U256, u32), Box<dyn std::error::Error>> {
    let (reserve0, reserve1, block_timestamp_last) = match pair_contract {
        Pair::UniswapV2Pair(uniswap_pair) => uniswap_pair.get_reserves().call().await?,
        Pair::SushiswapV2Pair(sushiswap_pair) => sushiswap_pair.get_reserves().call().await?,
    };
    Ok((
        U256::from(reserve0),
        U256::from(reserve1),
...
pub fn calculate_gas_cost(uni_gas: u64, sushi_gas: u64, gas_price_gwei: u64) -> U256 {
    let total_gas = uni_gas + sushi_gas;
    let total_gas_cost_ether = total_gas as f64 * gas_price_gwei as f64 * 1e-9; // Convert to Ether
    let total_gas_cost_wei = (total_gas_cost_ether * 1e18) as u64; // Convert to Wei
    U256::from(total_gas_cost_wei)
}

/*
You may also consider other optimization techniques, such as golden-section search or the Newton-Raphson method, to find the optimal trade amount.
Additionally, you can look into querying an Ethereum node to get the current gas prices for more accurate estimates. This can be done using eth_gasPrice JSON-RPC method,
 or by utilizing the web3 or ethers libraries if you integrate your Rust code with JavaScript or other languages that support these libraries. */
pub fn calculate_optimal_trade_amount(
    ...
    // Convert reserves to f64
    let reserve0_f64 = reserve0.to_string().parse::<f64>().unwrap();
    let reserve1_f64 = reserve1.to_string().parse::<f64>().unwrap();

    // Calculate the constant product invariant, k
    let k = reserve0_f64 * reserve1_f64;

    // Define the search space
    let mut lower_bound: f64 = 0.0;
    let mut upper_bound: f64 = reserve0_f64;
    let mut optimal_trade_amount = None;
    let mut max_profit = 0.0;

    for _ in 0..max_iterations {
        let trade_amount = (lower_bound + upper_bound) / 2.0;
        let new_reserve0 = reserve0_f64 + trade_amount;
        let new_reserve1 = k / new_reserve0;

        // Calculate the slippage
        let slippage = 1.0 - (reserve1_f64 / new_reserve1);

        // Calculate the profit
        let profit = (new_reserve1 - reserve1_f64) * slippage - gas_cost;

        // Check if the trade meets the minimum profit margin and has a higher profit than the current max profit
        if profit / new_reserve1 >= min_profit_margin && profit > max_profit {
            max_profit = profit;
            optimal_trade_amount = Some(U256::from(trade_amount as u64));

            // Narrow the search space
            lower_bound = trade_amount;
        } else {
            upper_bound = trade_amount;
        }
    }

    optimal_trade_amount
}
...
pub async fn get_amounts_out(
    amount_in: U256,
    path: Vec<Address>,
    router: &impl ContractCall<Provider<Ws>, (Vec<U256>,)>,
) -> Result<Vec<U256>, Box<dyn std::error::Error>> {
    let amounts_out = router
        .call("getAmountsOut", (amount_in, path), None, Options::default(), None)
        .await?;
